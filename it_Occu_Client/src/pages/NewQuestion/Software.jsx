...
function Software() {
  ...
  const { setSoftscore } = useScore();
  const [selectedRatings, setSelectedRatings] = useState([]);
  const [question_list, setQuestion] = useState([]);

  const fetchQuestion = async () => {
    try {
      const response = await axios.get(`https://itoccupa-production.up.railway.app/api/newquestion/2`);
  ...
  const handleSubmit = (e) => {
    ...
    if (Object.keys(selectedRatings).length === 10) {
      ...
      setSoftscore(suitabilityPercentage.toFixed(2));
      navigate("/successes");
    } else {
          ...
          <div className="sm:mx-auto sm:w-full sm:max-w-sm">
            <div className="text-center mt-10">
              ...
              <p className="mt-8 text-pretty text-xl font-medium text-gray-500 sm:text-md">
                กรุณาตอบคำถามทั้ง 10 ข้อเพื่อวัดความเหมาะสมในสายงาน Software Developer
              </p>
            </div>
          </div>
