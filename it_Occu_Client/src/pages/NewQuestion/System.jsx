...
function System() {
  ...
  const { setDatascore } = useScore();
  const [selectedRatings, setSelectedRatings] = useState([]);
  const [question_list, setQuestion] = useState([]);

  const fetchQuestion = async () => {
    try {
      const response = await axios.get(`https://itoccupa-production.up.railway.app/api/newquestion/3`);
  ...
  const handleSubmit = (e) => {
    ...
    if (Object.keys(selectedRatings).length === 10) {
      ...
      setDatascore(suitabilityPercentage.toFixed(2));
      navigate("/successes");
    } else {
